...
import { StudyResult, UserSettings } from './types';
...
const AppLayout: React.FC<{ settings: UserSettings; children: React.ReactNode }> = ({ settings, children }) => {
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto relative overflow-hidden">
      {!isScanning && (
        <header className="px-6 py-5 flex justify-between items-center z-30 shrink-0">
          <div className="flex items-center space-x-3 group cursor-pointer" onClick={() => navigate('/')}>
            <div className="w-10 h-10 bg-gradient-to-tr from-blue-600 to-indigo-500 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/30 group-hover:rotate-6 transition-transform">
              <span className="text-white font-black text-lg">S</span>
            </div>
            <h1 className="text-2xl font-800 text-white tracking-tight">StudyBuddy</h1>
          </div>
          <button 
      ...
      <main className="flex-1 overflow-y-auto pb-32 z-10 px-4">
        {children}
      </main>

      {!isScanning && (
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-[380px] nav-glass rounded-3xl flex justify-around items-center py-4 z-40 shadow-2xl border border-white/20">
          <button onClick={() => navigate('/')} className={`flex flex-col items-center transition-all ${location.pathname === '/' ? 'text-blue-600 scale-110' : 'text-gray-400 hover:text-gray-600'}`}>
          ...
          <button onClick={() => navigate('/scan')} className="w-16 h-16 bg-gradient-to-tr from-blue-600 to-indigo-600 rounded-2xl flex items-center justify-center -mt-12 shadow-xl shadow-blue-600/40 border-4 border-white text-white transition-all active:scale-90 hover:rotate-3">
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>
          </button>

          <button onClick={() => navigate('/planner')} className={`flex flex-col items-center transition-all ${location.pathname === '/planner' ? 'text-blue-600 scale-110' : 'text-gray-400 hover:text-gray-600'}`}>
            ...
            <span className="text-[10px] font-bold mt-1 uppercase tracking-tighter">Planner</span>
          </button>
        </nav>
      )}
    </div>
...
const App: React.FC = () => {
  ...
  const resetAllData = () => {
    ...
    localStorage.removeItem('study_buddy_settings');
  };
    ...
    <HashRouter>
      <AppLayout settings={settings}>
        <Routes>
          ...
          <Route path="/library" element={<Home results={results} settings={settings} isLibrary onDelete={deleteResult} />} />
