    ...
    <div className="flex flex-col h-full w-full max-w-md mx-auto relative overflow-hidden bg-transparent">
      {!isScanning && (
        <header className="px-6 py-4 flex justify-between items-center z-50 shrink-0 pt-[env(safe-area-inset-top,20px)]">
          ...
          <button onClick={() => navigate('/settings')} className="w-10 h-10 rounded-full border-2 border-blue-500/30 overflow-hidden bg-white/5">
            <img 
      ...
      <main className={`flex-1 overflow-y-auto relative scrollbar-hide ${isScanning ? '' : 'px-4 pb-32'}`}>
        {children}
      </main>

      {!isScanning && (
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-[380px] nav-glass rounded-3xl flex justify-around items-center py-4 z-50 border border-white/20 shadow-2xl mb-[env(safe-area-inset-bottom,0px)]">
          <button onClick={() => navigate('/')} className={`flex flex-col items-center ${location.pathname === '/' ? 'text-blue-400' : 'text-gray-400'}`}>
             <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
          </button>
          <button onClick={() => navigate('/scan')} className="w-14 h-14 bg-blue-600 rounded-2xl flex items-center justify-center -mt-10 text-white shadow-xl shadow-blue-600/40 border-4 border-[#020617] active:scale-90 transition-transform">
            <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>
          </button>
          <button onClick={() => navigate('/planner')} className={`flex flex-col items-center ${location.pathname === '/planner' ? 'text-blue-400' : 'text-gray-400'}`}>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      console.debug("Results storage load failed", e);
    }
  }, []);
  ...
  return (
    <HashRouter>
      <AppLayout settings={settings}>
        <Routes>
          <Route path="/" element={<Home results={results} settings={settings} onDelete={id => setResults(r => r.filter(x => x.id !== id))} />} />
          <Route path="/scan" element={<Scan setScans={setCurrentScans} />} />
          <Route path="/processing" element={<Processing scans={currentScans} results={results} onDone={r => setResults(prev => [r, ...prev])} onUpdate={(id, u) => setResults(r => r.map(x => x.id === id ? {...x, ...u} : x))} />} />
          <Route path="/results/:id" element={<Results results={results} onUpdate={(id, u) => setResults(r => r.map(x => x.id === id ? {...x, ...u} : x))} />} />
