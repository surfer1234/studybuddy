...
// Gebruik nu default imports zonder aliassen (@/)
...
const AppLayout: React.FC<{ settings: UserSettings; children: React.ReactNode }> = ({ settings, children }) => {
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto relative overflow-hidden bg-transparent">
      ...
      {!isScanning && (
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-[380px] nav-glass rounded-3xl flex justify-around items-center py-4 z-50 border border-white/20">
          ...
          <button onClick={() => navigate('/scan')} className="w-14 h-14 bg-blue-600 rounded-2xl flex items-center justify-center -mt-10 text-white">
